...
\section{Continuous-Time modeling}
	...
	\subsection{State equations}
		...
		\begin{align}
			...
			J_r \ddot{r}(t) &= K_m V_m p(t) - \tau_{rq}(t) - K_r \dot{r}(t)
		\end{align}
		where:
		\begin{itemize}
		...
		The coupling torque $\tau_{rq}$ between the rotor and the end shaft depends on the mechanical backlash and can be modeled using a piecewise model. Let $\Delta_b$ denote the half-width of the backlash dead zone, expressed at the output shaft.
		\begin{itemize}
	...
	\subsection{Model summary}
		...
		\subsubsection{Contact in the gearbox - $r(t)>q(t)$ - Control input greater than dry friction}
			...
			\begin{align}
				...
				K_mV_mp(t) &\ge \tau_{dM} \\
				\big( J_r + J_q \big) \ddot{q}(t) &= K_mV_mp(t) - \tau_{dM} - \big(K_r+K_q\big)\dot{q}(t)
			\end{align}
	...
	\subsection{State-space matrix form modelisation}
		...
		\subsubsection{State vectors}
			...
			\begin{align}
				...
				\tilde{p}^1(t) &= K_mV_mp(t) - \tau_{dM} \\
				\tilde{p}^2(t) &= K_mV_mp(t) \\
				\tilde{p}^3(t) &= K_mV_mp(t) + \tau_{dM} \\
